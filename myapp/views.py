...
#generateascii.py
import random
# https://www.amnetitsolution.co.th/
# http://localhost:8000/
def GenerateToken(domain='https://www.amnetitsolution.co.th/confirm/'):
...
def EmailConfirm(email,name,token):
	subject = 'ยืนยันการสมัครผู้แทนจำหน่าย amnetitsolution'
	newmember_name = name
	content = '''
...
def Home(request):
	product = Allproduct.objects.all().order_by('id').reverse()[:4]
	# product = Allproduct.objects.all()[:4] #ดึงข้อมุลมาจากฐานข้อมุลทั้งหมด
...
def MyCartEdit(request):
	...
	if request.method == 'POST':
		...
		# for ed in editlist:
		# 	edit = Cart.objects.get(productsid=ed[0],user=user) #productid
...
def Checkout(request):
	...
	if request.method == 'POST':
		...
		if page == 'confirm':
			...
			email = username
			subject = 'รหัสใบสั่งซื้อ : {} บริษัท แอมเน็ต ไอที โซลูชั่น แอนด์ เอ็นจิเนียริ่ง จำกัด'.format(orderid)
			
			content = '''
			ขอขอบคุณลูกค้าที่ไว้วางใจสั่งซื้อสินค้ากับทาง บริษัท แอมเน็ต ไอที โซลูชั่น แอนด์ เอ็นจิเนียริ่ง จำกัด
			'''
			
			sendthai(email,subject,content)
...
def OrderlistPage(request):
	...
	if request.method == 'POST':
		...
		item = OrderPending.objects.get(user=user,orderid=orderid)
		item.delete()
...
def UpdatePaid(request,orderid,status):
	...
	if status == 'confirm':
		...
		use = str(order.user) 
		email = use
		subject = 'รหัสใบสั่งซื้อ : {} สถานะ : กำลังจัดส่งสินค้า'.format(orderid)
		
		content = '''
		สินค้ากำลังจัดส่ง : กรุณาตรวจสอบหมายเลขการจัดส่งที่รายการสั่งซื้อ \n
		ขอขอบคุณลูกค้าที่ไว้วางใจสั่งซื้อสินค้ากับทาง บริษัท แอมเน็ต ไอที โซลูชั่น แอนด์ เอ็นจิเนียริ่ง จำกัด
		'''	
		sendthai(email,subject,content)
		####### ตัดสต๊อก #########
...
def PieChart(request):
	...
	odlist = OrderList.objects.all()

	total1 = sum ([ c.total for c in odlist])
		# total = sum([500,200])
	odlist.total1 = total1
	...
	count = len(products) #quantity of mobile
	total = sum(allprice)
	average = total / count
	mmax = max(allprice)
	mmin = min(allprice)
...
def Search(request):
	...
	if search_query:
		product = Allproduct.objects.filter(Q(name__icontains=search_query) | Q(namedetail__icontains=search_query))
	context = {'product':product}
	return render(request,'myapp/allproduct.html',context)
